...
class SingleFloatArduPar3 : public AbstractArduPar3
{
public:
  SingleFloatArduPar3() : value(0),
  ...
  void setup(
      ...
      float minValue,
      float maxValue,
		float startValue =0,																			///< value will be initialized with this
  ...
  float getValue() const { return *valuePointer; }
  float getMax()const {return maxValue;}
  float getMin()const {return minValue;}

  void setMax(float maxValue){this->maxValue=maxValue;*valuePointer=min(*valuePointer,maxValue);}
  void setMin(float minValue){this->minValue=minValue;*valuePointer=max(*valuePointer,minValue);}
  ...
  void setValueFromFloat(float value, size_t position = 0);   ///< set Value using a floating point Number (and possibly a some single component of a vector)
...
protected:
  ...
  float value;
  float *valuePointer = nullptr;               ///< points to the parameter value to be set
